...
function formatCaseStudies(data: CmsCaseStudy[]): CaseStudyCard[] {
...
export default function CaseStudiesClient({ initialCaseStudies }: { initialCaseStudies: CmsCaseStudy[] | null }) {
  const [caseStudies, setCaseStudies] = React.useState<CaseStudyCard[]>(() =>
    initialCaseStudies?.length ? formatCaseStudies(initialCaseStudies) : CASE_STUDIES,
  );

  React.useEffect(() => {
    // Server already supplied CMS data; only fetch in the browser when it could not.
    if (initialCaseStudies) return;
    fetchCmsCollection<CmsCaseStudy>("case-studies")
      .then((data) => {
        if (data.length > 0) setCaseStudies(formatCaseStudies(data));
      })
        ...
        <div className="max-w-7xl mx-auto relative">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            ...
            <motion.div custom={3} variants={fadeUp} initial="hidden" animate="visible" className="relative w-full aspect-square lg:aspect-[4/3] rounded-3xl overflow-hidden hidden md:block" style={{ border: "1px solid rgba(255,255,255,0.06)" }}>
              <Image src="/imagery/high-resolution-visualization.webp" alt="High-Resolution Visualization" fill className="object-cover" priority />
          ...
          {caseStudies.map((cs, i) => (
            <React.Fragment key={cs.slug}>
              {i === 2 && (
                ...
                >
                  <Image src="/imagery/high-res-screen-interaction.webp" alt="High-Res Screen Interaction" fill className="object-cover" />
                  <div className="absolute inset-0 bg-[#050505] opacity-20" />
              ...
              {i === 8 && (
                <motion.div variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-40px" }}
                  ...
                  style={{ border: "1px solid rgba(255,255,255,0.07)" }}
                >
                  <Image src="/imagery/digital-maze-exploration.webp" alt="Digital Maze Exploration" fill className="object-cover" />
                  <div className="absolute inset-0 bg-[#050505] opacity-20" />
                  <div className="absolute inset-0 bg-gradient-to-t from-[#050505] to-transparent opacity-60" />
                </motion.div>
              )}
