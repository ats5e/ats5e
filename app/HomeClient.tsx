...
const INSIGHT_FALLBACK_IMAGES = [
  ...
  "/imagery/futuristic-silhouettes.webp",
  "/imagery/high-resolution-visualization.webp",
  "/imagery/colorful-light-corridor.webp",
];
...
type HomeClientProps = {
  initialHomePage: CmsHomePage | null;
  initialInsights: CmsInsight[] | null;
  initialSolutions: CmsSolution[] | null;
};

export default function HomeClient({ initialHomePage, initialInsights, initialSolutions }: HomeClientProps) {
  const [homePageContent, setHomePageContent] = useState<HomePageContent>(() =>
    initialHomePage ? mergeHomePageContent(DEFAULT_HOME_PAGE_CONTENT, initialHomePage) : DEFAULT_HOME_PAGE_CONTENT,
  );
  const [latestInsights, setLatestInsights] = useState<HomeInsightPreview[]>(() => {
    const latest = initialInsights ? getLatestInsightPreviews(initialInsights) : [];
    return latest.length > 0 ? latest : DEFAULT_HOME_INSIGHTS.slice(0, 3);
  });

  // Server already supplied CMS data where it could; the browser only fetches what is missing.
  useEffect(() => {
...
function SolutionsSection({ content, initialSolutions }: { content: HomePageContent; initialSolutions: CmsSolution[] | null }) {
  const [solutions, setSolutions] = useState<SolutionSummary[]>(() =>
    initialSolutions?.length ? formatSolutions(initialSolutions) : SOLUTIONS,
  );
  ...
  useEffect(() => {
    ...
    fetchCmsCollection<CmsSolution>("solutions")
      .then((data) => {
        if (data.length > 0) setSolutions(formatSolutions(data));
      })
      ...
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center mb-20">
          ...
          <motion.div variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }}
            ...
            style={{ border: "1px solid rgba(255,255,255,0.06)" }}
          >
            <Image src="/imagery/colorful-light-corridor.webp" alt="Colorful Light Corridor" fill className="object-cover" />
            <div className="absolute inset-0 bg-[#050505] opacity-20" />
      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          <div aria-hidden className="absolute inset-0">
            <Image src="/imagery/colorful-pixel-gradient.webp" alt="Colorful Pixel Gradient" fill className="object-cover opacity-50 mix-blend-overlay" />
          </div>
