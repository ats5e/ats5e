...
function formatPartners(data: CmsPartner[]): PartnerCard[] {
    return sortByDisplayOrder(data).map((partner) => ({
        name: partner.name,
...
export default function PartnersClient({ initialPartners }: { initialPartners: CmsPartner[] | null }) {
    const [partners, setPartners] = React.useState<PartnerCard[]>(() =>
        initialPartners?.length ? formatPartners(initialPartners) : PARTNERS,
    );
    ...
    React.useEffect(() => {
        ...
        fetchCmsCollection<CmsPartner>("partners")
            .then((data) => {
                if (data.length > 0) setPartners(formatPartners(data));
            })
